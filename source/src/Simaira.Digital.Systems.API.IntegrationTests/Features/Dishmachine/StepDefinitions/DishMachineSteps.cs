namespace Simaira.Digital.Systems.IntegrationTests.Features.Dishmachine.StepDefinitions
{
    using System;
    ...
    using Simaira.Digital.Systems.IntegrationTests.Features.Landing.StepDefinitions;
    using Microsoft.VisualStudio.TestTools.UnitTesting;
    using TechTalk.SpecFlow;

    [Binding]
    public class DishMachineSteps : BaseSteps
    {
        public DishMachineSteps(IEcolabCustomerPortalClientFactory customerPortalClientFactory, ScenarioContext scenarioContext, Endpoints endpoints)
            : base(customerPortalClientFactory, scenarioContext, endpoints)
        {
        }

        [Given(@"Get aligned CDM Sites and Accounts for '(.*)' from CDM API for Dish Machine")]
        ...
        public void CraeteDishMachineOverallPerformancePayload()
        {
            ...
            var dishMachines = (IEnumerable<ServiceAreaDetail>)GetFromScenarioContext(DishMachineServiceAreasKey);
            ...
            foreach (var email in emailList)
            {
                ...
                {
                    if (!chartRequests.Any(cdm => cdm.Value == Convert.ToString(cdmAccount.SiteKey)))
                    {
                        ...
                        var filter = dishMachines.Where(data => data.AccountNumber == cdmAccount.AccountNumber).ToList();
            ...
            AddToScenarioContext(DishMachineOverviewWidgetKey, overviewWidget);
        }

        [Then(@"Get Dish Machine Overall Performance for Pulse Check '(.*)'")]
        public async Task GetDishMachineOverallPerformanceAsync(bool isPulseCheck)
        {
            var response = (OverviewWidget)GetFromScenarioContext(DishMachineOverviewWidgetKey);
            response.IsPulseCheck = isPulseCheck;

            string baseUrl = _endpoints.CustomerPortalEndpoint;
            RequestPayload payload = new RequestPayload();
            payload.PageContext = new Ecolab.Simaira.Digital.CustomerPortal.Model.RequestModels.FeatureContext { Name = "dishMachine", Component = "dishMachineOverallPerformance" };
            payload.DishMachine = response;
            var result = await _customerPortalClientFactory.HttpPostAsync<Response<IEnumerable<OverallPerformanceResponseDto>>>(baseUrl, payload).ConfigureAwait(false);
            Assert.AreEqual(200, result.StatusCode);
            AddToScenarioContext(DishMachinePulseCheckKey, result);

            if (isPulseCheck)
            {
                Assert.AreEqual("Overview", result.Value.First().DeviceGroupName);
